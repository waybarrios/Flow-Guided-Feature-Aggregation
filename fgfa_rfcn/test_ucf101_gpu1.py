...
def test_net(cfg, dataset, image_set, ctx, prefix, epoch, shuffle):
    ...
    for sample_idx, roidb in enumerate(gtviddb):
        ...
        test_datas = TestLoader([roidb], cfg, batch_size=1, shuffle=shuffle, ctx=[ctx[1]])
        for nbatch, data_batch in enumerate(test_datas):
            feat_predictors = get_predictor(feat_sym, feat_sym_instance, cfg, arg_params, aux_params, test_datas, [ctx[1]])
        ...
        print('{0}--->{1}'.format(roidb['video_name'], prediction_name))

        final_outputs.append(results)


    print('writing the prediction results...')
    with open(os.path.join(cache_path, 'test_part2.pkl'), 'wb') as f:
